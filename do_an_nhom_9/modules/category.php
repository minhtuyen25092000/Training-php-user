...
class Category extends Db{
    ...
    public function getAllProductOfCategory($id){
        $sql = self::$connection->prepare("SELECT * FROM `products`,`category` WHERE `products`.`cate_id`= `category`.`cate_id` and `category`.`cate_id` =$id");
        return $this->select($sql);
    }
}